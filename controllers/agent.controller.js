...
// Agents change rarely but are read on nearly every dashboard load → cache the
// list for a short window. Any write clears it so the next read is fresh.
const AGENTS_TTL = 30 * 1000;
const agentsKey = (companyId) => `agents:${companyId || 'global'}`;
const invalidateAgents = () => cache.invalidate('agents:');
...
const seedDefaultAgents = async () => {
  ...
  try {
    ...
    logger.error('Error seeding default agents', { err: error.message });
  }
};
...
const getAgents = async (req, res) => {
  try {
    ...
    const agents = await cache.wrap(agentsKey(companyId), AGENTS_TTL, () =>
      Agent.find(filter).select('-password').sort({ createdAt: -1 }).lean()
    );
    res.json(agents);
  } catch (error) {
    logger.error('Error fetching agents', { err: error.message });
...
const createAgent = async (req, res) => {
  try {
    ...
    logger.error('Error creating agent', { err: error.message });
...
const updateAgent = async (req, res) => {
  try {
    ...
    logger.error('Error updating agent', { err: error.message });
...
const deleteAgent = async (req, res) => {
  try {
    ...
    logger.error('Error deleting agent', { err: error.message });
...
const updateAgentPassword = async (req, res) => {
  try {
    ...
    logger.error('Error resetting agent password', { err: error.message });
    res.status(500).json({ message: 'Server error, failed to reset password' });
  }
};

// Agent sets their own real-time availability (online/offline/busy).
const updateAvailability = async (req, res) => {
  try {
    ...
    // Let dashboards reflect presence changes live.
    try {
      const { getIO } = require('../socket/socket');
      const io = getIO();
      if (io) {
        const room = req.companyId ? String(req.companyId) : 'global';
        io.to(room).emit('agent_availability', { agentId: req.agentId, availability });
      }
    } catch (err) {
      logger.error('Failed to emit agent_availability', { err: err.message });
    }

    res.json(agent);
  } catch (error) {
    logger.error('Error updating availability', { err: error.message });
    res.status(500).json({ message: 'Server error, failed to update availability' });
  }
};

module.exports = { seedDefaultAgents, getAgents, createAgent, updateAgent, deleteAgent, updateAgentPassword, updateAvailability };
