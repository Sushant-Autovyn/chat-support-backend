...
const createCompany = async (req, res) => {
  try {
    ...
    if (error.code !== 11000) logger.error('Error creating company', { err: error.message });
    if (error.code === 11000) {
...
const getCompany = async (req, res) => {
  try {
    ...
    logger.error('Error fetching company', { err: error.message });
...
const updateCompanySettings = async (req, res) => {
  try {
    ...
    logger.error('Error updating company settings', { err: error.message });
    res.status(500).json({ message: 'Server error' });
  }
};
